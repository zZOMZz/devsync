...
首次同步或规则/目标变更时，展示新增、覆盖和删除清单；确认后先备份将被覆盖/删除的远端文件，再同步。备份失败会终止同步。`--yes` 允许明确接受首次接入或规则变更后的同步，仍会执行预览、备份和校验。
...
- 备份恢复：先 `devsync stop`，将远端 `项目目录.sync-backup-时间.tar.gz` 解压到独立目录核对后恢复。备份只包含接入时会被覆盖/删除的文件。

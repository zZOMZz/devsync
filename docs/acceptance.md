...
### controller disabled 修复回归（2026-09-09）

在两个隔离的真实 Mutagen 项目中删除一个暂停会话的归档，复现 terminate 失败后控制器保持 disabled。修复后 pause 可以恢复磁盘记录已消失的残留会话，terminate 可以处理已完成磁盘删除的缺失文件错误，重新创建的会话可正常传输，另一个项目保持运行。配置和凭据保持原样；权限错误及仍有磁盘记录的会话不会触发丢弃式恢复。服务回归确认原始错误不会被暂停清理或警告输出错误覆盖。完整回归 141 项通过，无跳过。

...
macOS ARM64、Node.js 24.14.0 上开启真实 SSH、Mutagen 和 PTY 测试，137 项全部通过，无跳过。新增覆盖跨进程索引并发写入、路径去重、目录丢失与重新定位、慢查询超时、面板 JSON、选择项目开启/停止、配置与确认切换、删除记录、窗口缩放、Ctrl+C/SIGTERM 及向导内中断。真实引擎确认停止一个项目不影响另一个、退出查询不停止既有 worker；离线安装的包可加载 Ink 和 React。Windows/Linux 终端实机、目录移动后的运行状态迁移仍未验证或实现。

...
Mutagen 0.18.1 在 terminate 删除会话/归档失败时，可能保留已禁用的内存控制器。Session 的暂停恢复仅在该控制器的会话磁盘记录已不存在时重启项目 daemon；terminate 的缺失文件恢复还要求归档不存在。持久会话和权限错误不会被当作可丢弃状态，恢复不会清空状态目录或重启其他项目。同步主流程失败后，即使暂停清理也失败，仍返回原始错误，并附加 cleanupError 诊断。

...
当前交互向导与服务保存前都会验证连接和目录，因此存在重复探测，属于待优化的边界。
...
flowchart TD
    ...
    Confirm -->|继续| Backup[备份会被覆盖或删除的远端文件]
...
备份仅发生在需要重新确认接入的路径中，并且只包含当次将被覆盖或删除的文件。日常每次文件修改不是一次备份。没有受影响文件时不生成空备份。

指纹当前包含本地根目录、远端配置、`identityFile`、同步模式、轮询策略和项目规则，不包含 Mutagen 发行版本。
...
`status` 设置 `MUTAGEN_DISABLE_AUTOSTART=1`，防止查询状态时拉起后台服务。`auto` 表示管理进程状态，不应独立当作文件已对齐的证明。
